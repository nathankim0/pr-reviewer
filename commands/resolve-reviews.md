...
description: PR 미해결 리뷰 코멘트를 분석하고 코드를 수정하고 답글을 게시합니다
---
...
**Outdated 스레드 처리**:
`isOutdated === true`인 스레드는 해당 라인의 코드가 변경되었다는 의미이지만, 리뷰어의 요청이 실제로 반영되었다는 보장은 아닙니다. Step 2에서 현재 코드를 직접 읽고 코멘트의 의도와 대조하여 정확히 분류합니다.
...
- 코멘트 내용과 맥락
- **현재 코드 상태** (파일을 Read하여 코멘트가 지적한 부분의 현재 코드를 직접 확인)
- **코멘트 의도 대조**: 리뷰어가 요청한 변경이 현재 코드에 실제로 반영되었는지 판단 (라인이 변경되었더라도 요청과 다른 방향으로 수정되었으면 `code_change`로 분류)
- GitHub suggestion 블록 포함 여부
- `isOutdated` 여부 (참고 정보로만 활용, 이것만으로 `already_done` 판단 금지)
...
**파일별 병렬 처리** (Task 에이전트 사용, **model: "sonnet"**):
