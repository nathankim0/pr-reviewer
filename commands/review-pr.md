...
4. "코드 품질만" - 네이밍, 구조, 가독성, SOLID 원칙, 코드 냄새 등에 집중합니다
...
**SOLID 원칙 위반**:
- **SRP (단일 책임)**: 하나의 클래스/함수가 여러 책임을 가지는지 (예: 데이터 가공 + UI 렌더링 + API 호출을 한 함수에서 처리)
- **OCP (개방-폐쇄)**: 기존 코드 수정 없이 확장 가능한 구조인지 (예: if/switch 분기가 타입 추가마다 수정 필요)
- **LSP (리스코프 치환)**: 하위 타입이 상위 타입의 계약을 위반하는지 (예: 오버라이드 메서드가 예상과 다른 동작)
- **ISP (인터페이스 분리)**: 사용하지 않는 메서드에 의존하는지 (예: 거대 인터페이스를 구현하면서 대부분 빈 메서드)
- **DIP (의존성 역전)**: 구체 구현에 직접 의존하는지 (예: 고수준 모듈이 저수준 모듈을 직접 import)

**코드 냄새 탐지**:
- **Long Method / Large Class**: 과도하게 긴 함수나 클래스
- **Feature Envy**: 다른 클래스의 데이터를 과도하게 사용하는 메서드
- **Shotgun Surgery**: 하나의 변경이 여러 파일을 동시에 수정해야 하는 구조
- **God Object**: 너무 많은 역할을 담당하는 클래스/모듈
- **Primitive Obsession**: 원시 타입 남용 (의미 있는 도메인 타입 미사용)

**주의**: SOLID 위반이나 코드 냄새는 PR diff의 변경 범위 내에서만 지적하세요. 기존 코드의 구조적 문제를 전부 지적하지 말고, **이번 PR에서 새로 도입되거나 악화된** 문제만 보고하세요.
